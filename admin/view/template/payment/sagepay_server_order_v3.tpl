...
<div class="success" id="sagepay_server_transaction_msg" style="display:none;"></div>
<table class="form">
  <tr>
    <td><?php echo $text_order_ref; ?></td>
    <td><?php echo $sagepay_server_v3_order['VendorTxCode']; ?></td>
  </tr>
  <tr>
    <td><?php echo $text_order_total; ?></td>
    <td><?php echo $sagepay_server_v3_order['total_formatted']; ?></td>
  </tr>
  <tr>
    <td><?php echo $text_total_released; ?></td>
    <td id="sagepay_server_total_released"><?php echo $sagepay_server_v3_order['total_released_formatted']; ?></td>
  </tr>
  <tr>
    <td><?php echo $text_release_status; ?></td>
    <td id="release_status">
      <?php if ($sagepay_server_v3_order['release_status'] == 1) { ?>
        <span class="release_text"><?php echo $text_yes; ?></span>
      <?php } else { ?>
        <span class="release_text"><?php echo $text_no; ?></span>&nbsp;&nbsp;
        <?php if ($sagepay_server_v3_order['void_status'] == 0) { ?>
          <input type="text" width="10" id="release_amount" value="<?php echo $sagepay_server_v3_order['total']; ?>"/>
  ...
  <tr>
    ...
    <td id="void_status">
      <?php if ($sagepay_server_v3_order['void_status'] == 1) { ?>
        <span class="void_text"><?php echo $text_yes; ?></span>
      <?php } elseif($sagepay_server_v3_order['void_status'] == 0 && $sagepay_server_v3_order['release_status'] != 1 && $sagepay_server_v3_order['rebate_status'] != 1) { ?>
  ...
  <tr>
    ...
    <td id="rebate_status">
      <?php if ($sagepay_server_v3_order['rebate_status'] == 1) { ?>
        <span class="rebate_text"><?php echo $text_yes; ?></span>
      <?php } else { ?>
        <span class="rebate_text"><?php echo $text_no; ?></span>&nbsp;&nbsp;

        <?php if ($sagepay_server_v3_order['total_released'] > 0 && $sagepay_server_v3_order['void_status'] == 0) { ?>
  ...
  <tr>
    ...
    <td>
      <table class="list" id="sagepay_server_transactions">
        <thead>
          <tr>
        ...
        <tbody>
          <?php foreach($sagepay_server_v3_order['transactions'] as $transaction) { ?>
            <tr>
  ...
  $("#btn_void").click(function () {
    if (confirm('<?php echo $text_confirm_void; ?>')) {
      $.ajax({
        ...
        url: 'index.php?route=payment/sagepay_server_v3/void&token=<?php echo $token; ?>',
        beforeSend: function() {
          $('#btn_void').hide();
  ...
  $("#btn_release").click(function () {
    if (confirm('<?php echo $text_confirm_release; ?>')) {
      $.ajax({
        ...
        url: 'index.php?route=payment/sagepay_server_v3/release&token=<?php echo $token; ?>',
        beforeSend: function() {
          $('#btn_release').hide();
          $('#release_amount').hide();
  ...
  $("#btn_rebate").click(function () {
    if (confirm('<?php echo $text_confirm_rebate ?>')) {
      $.ajax({
        ...
        url: 'index.php?route=payment/sagepay_server_v3/rebate&token=<?php echo $token; ?>',
        beforeSend: function() {
          $('#btn_rebate').hide();
          $('#rebate_amount').hide();
